...
function testSkeleton() {
  Logger.log('═══════════════════════════════════════════════════════');
  Logger.log('       DOCUMENT SKELETON TEST v2 - Visual Heuristics');
  Logger.log('═══════════════════════════════════════════════════════');

  var skeleton = getDocSkeleton();

  if (!skeleton || skeleton.error) {
    Logger.log('❌ ERROR: ' + (skeleton ? skeleton.error : 'null'));
    return;
  }

  Logger.log('');
  Logger.log('📊 ESTADÍSTIQUES:');
  Logger.log('   ⏱️ Temps scan: ' + skeleton.stats.scan_time_ms + 'ms ' +
              (skeleton.stats.scan_time_ms < 800 ? '✅' : '⚠️'));
  Logger.log('   📄 Document: ' + skeleton.doc_name);
  Logger.log('   📏 Total chars: ' + skeleton.stats.total_chars);
  Logger.log('   📝 Paràgrafs: ' + skeleton.stats.paragraph_count);
  Logger.log('   🗂️ Items estructura: ' + skeleton.stats.structure_items);
  if (skeleton.stats.truncated) {
    Logger.log('   ⚠️ TRUNCAT per temps!');
  }

  Logger.log('');
  Logger.log('🗂️ ESTRUCTURA DETECTADA:');
  ...
  skeleton.structure.forEach(function(item, i) {
    if (item.type === 'SECTION') {
      sectionCount++;
      Logger.log('   [' + i + '] 📄 SECTION: "' + item.preview.substring(0, 50) + '..."');
      if (item.entities && item.entities.length > 0) {
        Logger.log('        └─ Entities: ' + item.entities.join(', '));
      }
    } else if (item.type === 'WARNING') {
      Logger.log('   [' + i + '] ⚠️ ' + item.text);
    } else {
      headingCount++;
  ...
  Logger.log('');
  Logger.log('📈 RESUM:');
  Logger.log('   Headings trobats: ' + headingCount +
              (headingCount === 0 ? ' ⚠️ Cap heading!' : ' ✅'));
  Logger.log('   Seccions: ' + sectionCount);

  Logger.log('');
  Logger.log('═══════════════════════════════════════════════════════');
  if (skeleton.stats.scan_time_ms < 500 && headingCount > 0) {
    Logger.log('✅ APTE PER PRODUCCIÓ');
  } else if (skeleton.stats.scan_time_ms < 800) {
    Logger.log('🟡 ACCEPTABLE (optimitzable)');
  } else {
    Logger.log('🔴 NECESSITA OPTIMITZACIÓ');
  }
  Logger.log('═══════════════════════════════════════════════════════');
...
/**
 * Test manual per applyAutoStructure
 */
function testAutoStructure() {
  Logger.log('═══════════════════════════════════════════════════════');
  Logger.log('       AUTO-STRUCTURE TEST v1');
  Logger.log('       Convertir Visual Headings → H2');
  Logger.log('═══════════════════════════════════════════════════════');

  // Primer, mostrar estat ABANS
  Logger.log('');
  Logger.log('📊 ESTAT ABANS:');
  var skeletonBefore = getDocSkeleton();
  var headingsBefore = skeletonBefore.structure.filter(function(item) {
    return item.type !== 'SECTION' && item.type !== 'WARNING';
  }).length;
  Logger.log('   Headings natius: ' + headingsBefore);

  // Executar Auto-Structure
  Logger.log('');
  Logger.log('🔧 EXECUTANT AUTO-STRUCTURE...');
  var result = applyAutoStructure();
  ...
  Logger.log('   ⏱️ Temps: ' + result.execution_time_ms + 'ms');
  Logger.log('   ✅ Convertits a H2: ' + result.converted);
  Logger.log('   ⏭️ Saltats (ja eren headings): ' + result.skipped);
  Logger.log('   ❌ Errors: ' + result.errors);

  if (result.details.length > 0) {
    Logger.log('');
    Logger.log('📝 DETALLS DELS CANVIS:');
    result.details.forEach(function(detail, i) {
      Logger.log('   [' + detail.index + '] "' + detail.text + '" (' + detail.reason + ')');
    });
  }

  // Mostrar estat DESPRÉS
  Logger.log('');
  Logger.log('📊 ESTAT DESPRÉS:');
  var skeletonAfter = getDocSkeleton();
  var headingsAfter = skeletonAfter.structure.filter(function(item) {
    return item.type !== 'SECTION' && item.type !== 'WARNING';
  }).length;
  Logger.log('   Headings natius: ' + headingsAfter);

  Logger.log('');
  Logger.log('═══════════════════════════════════════════════════════');
  if (result.converted > 0) {
    Logger.log('✅ AUTO-STRUCTURE COMPLETAT');
    Logger.log('   ' + headingsBefore + ' → ' + headingsAfter + ' headings (+' + result.converted + ')');
  } else {
    Logger.log('ℹ️ CAP CANVI NECESSARI');
    Logger.log('   El document ja té estructura correcta o no té visual headings');
  }
  Logger.log('═══════════════════════════════════════════════════════');
